...
data_frame.drop(['BMI_category', 'tulineno', 'tucaseid', 'euinclvl', 'eufinlwgt', 'ethgt',
                 'euhgt', 'etwgt', 'exincome1', 'erhhch'],
                axis=1, inplace=True)
...
# Feature Selection
selection = SelectPercentile(percentile=88)
...
# Applying KNN
logistic_regression = LogisticRegression(C=1e5, max_iter=1000)
...
logistic_regression.fit(x_train_compressed, y_train)
y_train_predict = logistic_regression.predict(x_train_compressed)
y_test_predict = logistic_regression.predict(x_test_compressed)
...
show_pr_curve(logistic_regression, x_test_compressed, y_test)
show_roc_curve(y_train, y_train_predict, label='Logistic Regression (Training Set)')
show_roc_curve(y_test, y_test_predict, label='Logistic Regression (Test Set)')
