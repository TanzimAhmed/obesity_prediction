...
# Feature Selection
selection = SelectPercentile(percentile=98)
...
# Applying KNN
r_forest = RandomForestClassifier(n_estimators=5, random_state=0)
...
r_forest.fit(x_train_compressed, y_train)
y_train_predict = r_forest.predict(x_train_compressed)
y_test_predict = r_forest.predict(x_test_compressed)
...
show_pr_curve(r_forest, x_test_compressed, y_test)
show_roc_curve(y_train, y_train_predict, label='Random Forest (Training Set)')
show_roc_curve(y_test, y_test_predict, label='Random Forest (Test Set)')
